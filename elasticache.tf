...
resource "kubernetes_secret" "redis_secrets" {
  ...
  data = {
    ...
    REDIS_AUTH                           = var.redis_token_enabled ? random_uuid.redis_token[0].result : ""
    REDIS_URL                            = var.redis_token_enabled ? "redis${var.redis_transit_encryption_enabled ? "s" : ""}://:${random_uuid.redis_token[0].result}@${var.redis_cluster_mode["enabled"] ? aws_elasticache_replication_group.redis_cluster[0].primary_endpoint_address : aws_elasticache_replication_group.redis[0].primary_endpoint_address}:${var.redis_port}" : "redis${var.redis_transit_encryption_enabled ? "s" : ""}://${var.redis_cluster_mode["enabled"] ? aws_elasticache_replication_group.redis_cluster[0].primary_endpoint_address : aws_elasticache_replication_group.redis[0].primary_endpoint_address}:${var.redis_port}"
  }
}
